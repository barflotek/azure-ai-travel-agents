...
export interface FinanceTask {
  type: 'categorize_expense' | 'generate_report' | 'analyze_transaction' | 'budget_forecast' | 'invoice_generate';
  amount?: number;
  description?: string;
  date?: string;
  transactions?: any[];
  period?: 'daily' | 'weekly' | 'monthly' | 'quarterly' | 'yearly';
  reportType?: 'profit_loss' | 'expense_summary' | 'cash_flow' | 'budget_analysis';
  invoiceData?: {
    clientName: string;
...
export class FinanceAgent {
  ...
  async processTask(task: FinanceTask): Promise<any> {
  ...
  private async categorizeExpense(task: FinanceTask) {
    const prompt = `
...
`;
  ...
  private async generateReport(task: FinanceTask) {
    const prompt = `
...
`;
  ...
  private async analyzeTransaction(task: FinanceTask) {
    const prompt = `
...
`;
  ...
  private async budgetForecast(task: FinanceTask) {
    const prompt = `
...
`;
  ...
  private async generateInvoice(task: FinanceTask) {
    const { invoiceData } = task;
    if (!invoiceData) throw new Error('Invoice data required');
...
`;
  ...
  // Helper methods
  private parseFinanceResponse(response: any) {
    try {
  ...
  private assessRiskLevel(content: string): 'low' | 'medium' | 'high' {
    ...
    const riskCount = riskWords.filter(word => 
