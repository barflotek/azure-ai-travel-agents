...
export class EmailAgent {
    ...
    async checkEmails() {
        ...
        const summaries = await Promise.all(emailsToSummarize.map(email => this.summarizeRealEmail(email)));
        return {
    ...
    async getEmailsFast(maxResults = 50) {
        if (!this.gmailClient) {
            throw new Error('Gmail not connected. Please authenticate first.');
        }
    ...
    async getEmailsProgressive(maxResults = 50) {
        if (!this.gmailClient) {
            throw new Error('Gmail not connected. Please authenticate first.');
        }
    ...
    extractFromFromSnippet(snippet) {
        if (!snippet)
    ...
    extractSubjectFromSnippet(snippet) {
        if (!snippet)
            return 'No Subject';
    ...
    async sendRealEmail(task) {
        ...
        const result = await this.gmailClient.sendEmail(task.recipient, task.subject, task.content);
        return result;
    }
    ...
    async summarizeRealEmail(email) {
        const prompt = `
...
`;
    ...
    async generateAIReply(emailId, replyType) {
        if (!this.gmailClient) {
            throw new Error('Gmail not connected. Please authenticate first.');
        }
    ...
    // Bulk email actions
    async performBulkAction(emailIds, action) {
        if (!this.gmailClient) {
            throw new Error('Gmail not connected. Please authenticate first.');
        }
        const results = [];
...
`;
    ...
    async categorizeEmailTask(task) {
        const prompt = `
